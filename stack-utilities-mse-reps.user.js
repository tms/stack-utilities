...
inject(function ($) {
    ...
    container.find('.us-rep-block-switch').on('click', (function () {
        ...
        function change(reputation) {
            container.toggleClass('se', toggled);
            ...
            if (toggled) {
                ...
                display.prop('className', 'reps reputation-score');
            } else {
    ...
    })()).trigger('click').toggle(!!accept.text);
    ...
    function getReputation(callback) {
        ...
        $.get('https://api.stackexchange.com/2.1/users/' + user + '?site=meta.stackexchange&filter=' + userFilter + '&key=' + apiKey, function (response) {
    ...
    function repNumber(a) {
        ...
        if (1E4 > a) {
            a = a.toString();
            return a.substring(0, 1) + ',' + a.substring(1, 4);
        }         
        if (1E5 > a) {
